...
import { z } from "zod";
import { nerveJsonRpc, nerveRest, getBaseUrl, } from "./nerve-client.js";
...
// --- Chain & network ---
server.registerTool("nerve_info", {
...
server.registerTool("nerve_get_latest_height", {
    description: "Get the latest main chain block height for a chain",
    inputSchema: { chainId: chainIdSchema },
}, async ({ chainId }) => {
    const result = await nerveJsonRpc("getLatestHeight", [chainId]);
    return textContent(String(result));
});
server.registerTool("nerve_get_best_block_header", {
...
// --- Account & balance ---
server.registerTool("nerve_validate_address", {
    ...
    const result = await nerveJsonRpc("validateAddress", [chainId, address]);
    return jsonContent({ valid: result });
});
server.registerTool("nerve_get_account_balance", {
...
server.registerTool("nerve_get_balance_list", {
...
// --- Blocks ---
server.registerTool("nerve_get_block_by_height", {
...
server.registerTool("nerve_get_block_by_hash", {
...
server.registerTool("nerve_get_header_by_height", {
...
// --- Transactions ---
server.registerTool("nerve_get_tx", {
...
// --- Consensus / agents ---
server.registerTool("nerve_get_agent_list", {
...
server.registerTool("nerve_get_deposit_list", {
...
server.registerTool("nerve_jsonrpc", {
    description: "Call any Nerve JSON-RPC method by name with params array. Use for methods not covered by dedicated tools (e.g. getRandomSeedByCount, getProposalInfo, swap/farm APIs).",
    inputSchema: {
...
server.registerTool("nerve_rest", {
    description: "Call Nerve REST API by path. Method GET by default; pass body for POST. Base URL is from NERVE_API_BASE_URL (default https://api.nerve.network). Example paths: api/info, api/block/header/height/1000, api/tx/{hash}",
    inputSchema: {
